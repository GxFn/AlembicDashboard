...
import api, {
  type KnowledgeGap,
  type PanoramaArchitectureLayer as PanoramaLayer,
  type PanoramaHealth,
  type PanoramaLayerModule as LayerModule,
...
const TAB_CONFIG: { key: PanoramaTab; icon: React.ElementType; labelKey: string }[] = [
  ...
  { key: 'dependencies', icon: GitBranch, labelKey: 'panorama.dependencies' },
  { key: 'graph', icon: Share2, labelKey: 'panorama.graph' },
...
const StatsRow: React.FC<{
  ...
  const stats = useMemo(() => {
    ...
    if (health) {
      ...
      const pct = `${normalizePercent(dimCov)}%`;
      items.push(
        { label: t('panorama.coverage'), value: pct, icon: BarChart3, color: coverageTextColor(dimCov) },
    ...
    <div>
      ...
      <div className="flex flex-col gap-2">
        {sortedLayers.map((layer, idx) => {
          ...
          const totalRecipes = layer.modules.reduce((s, m) => s + (m.recipeCount ?? 0), 0);
          const hasUnknownRecipes = layer.modules.some((m) => m.recipeCount === null);
            ...
            <div key={layer.level} className="w-full">
              ...
              >
                ...
                <div className="flex items-center gap-3 text-xs text-[var(--text-secondary)] shrink-0 ml-3">
                  ...
                  {hasUnknownRecipes && (
                    <span className="bg-amber-500/15 text-amber-500 rounded-full px-2 py-0.5 font-medium" title={t('panorama.recipeCountDegraded')}>
                      {t('panorama.recipeCountUnavailable')}
                    </span>
                  )}
                </div>
              </button>
              ...
              {isExpanded && layer.modules.length > 0 && (
                <div className="mt-1.5 ml-5 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-1.5">
                  {layer.modules.map((mod) => {
                    ...
                    const recipeCount = moduleRecipeCount(mod);
                    const cov = recipeCount !== null && mod.fileCount > 0 ? Math.round((recipeCount / mod.fileCount) * 100) : 0;
                    return (
                      <div
                        key={mod.moduleId || mod.name}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5 mt-1">
                          ...
                          {recipeCount === null && (
                            <span className="text-[10px] font-medium text-amber-500" title={t('panorama.recipeCountDegraded')}>
                              {t('panorama.recipeCountUnavailable')}
                            </span>
                          )}
                        </div>
...
function coverageTextColor(ratio: number): string {
  const pct = normalizePercent(ratio);
...
function moduleRecipeCount(module: LayerModule): number | null {
  return module.recipeCount === null ? null : module.recipeCount;
}
